...
export default function Support() {
  ...
  const sub = query.app ? `${query.app} Support Request` : ''
  ...
  const submit = () => {
    if (name && email && message) {
      ...
      const templateId = 'template_2flqckh'
      ...
      if (templateParams.subject == '') {
        templateParams.subject = `New Support Request from ${name}`
      }
  ...
  return (
    <>
      <PageSEO title={`Support | ${siteMetadata.author}`} description={siteMetadata.description} />
      <div className="divide-y divide-gray-200 dark:divide-gray-700">
        ...
        <div className="flex w-full items-center justify-center pt-8">
          <div className="top-40 w-full max-w-2xl rounded-3xl border-2 border-gray-200 border-opacity-60 bg-white py-8 px-8 transition duration-500 dark:border-gray-700 dark:bg-gray-800">
            ...
            <p className="md:text-md font-regular p-2 text-center text-sm leading-7 text-gray-700 dark:text-white">
            ...
            <div className="mt-8 flex w-full flex-col">
              ...
              >
                {subject == `${query.app} Support Request` ? 'Subject (Read-only)' : 'Subject'}
              </label>
              <input
                arial-label="Enter a subject (optional)"
                type="text"
                className="focus:oultine-none mt-4 rounded-lg border border-gray-200 bg-gray-100 p-3 text-base leading-none text-gray-900 placeholder-gray-400 transition duration-500 focus:border-indigo-700 disabled:opacity-50 dark:border-gray-700 dark:bg-gray-900 dark:text-white dark:placeholder-gray-400"
                placeholder="Example Subject"
                value={subject}
                id="subject"
                disabled={subject == `${query.app} Support Request`}
